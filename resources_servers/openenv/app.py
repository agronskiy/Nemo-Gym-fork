...
class OpenEnvResourcesServer(SimpleResourcesServer):
    ...
    def _make_mcp_handler(self, tool_name: str, request_model: type):
        ...
        async def handler(request: Request, body: request_model) -> dict:
            ...
            try:
                ...
                obs = await session.env.step_async(action)
    ...
    # TODO(ahmadki): needs a revisit, is this even the right approach ? should we let the enviornment handle this ?
    # one example where current implementation doesn't work if the environment returns cumulative rewards instead of per-step deltas
