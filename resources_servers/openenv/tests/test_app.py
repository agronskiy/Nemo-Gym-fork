...
def _make_mock_env(reset_obs=None, step_obs=None, step_side_effect=None):
    """Create a mock OpenEnv environment with sync and async methods.

    Endpoint handlers call step_async/reset_async (AsyncMock).
    MCP tool discovery at startup calls sync step/reset (MagicMock).
    """
    env = MagicMock()
    if reset_obs is None:
        reset_obs = MagicMock(reward=0.0, done=False)
    env.reset.return_value = reset_obs
    env.reset_async = AsyncMock(return_value=reset_obs)
    if step_side_effect is not None:
        env.step.side_effect = step_side_effect
        env.step_async = AsyncMock(side_effect=step_side_effect)
    elif step_obs is not None:
        env.step.return_value = step_obs
        env.step_async = AsyncMock(return_value=step_obs)
    env.close = MagicMock()
    return env
...
class TestNonMCPStepEndpoint:
    def test_step_endpoint_calls_env_step_async(self) -> None:
        """POST /step should call env.step_async() with the correct Action type."""
        ...
        assert response.status_code == 200
        mock_env_instance.step_async.assert_called_once()
...
class TestErrorHandling:
    def test_step_after_done_returns_error(self) -> None:
        """Tool calls after done=True should return an error, not call env.step_async()."""
        ...
        client.post("/step", json={}, cookies=cookies)
        assert mock_env_instance.step_async.call_count == 1

        # Second step should NOT call env.step_async
        response = client.post("/step", json={}, cookies=cookies)
        data = response.json()
        assert "error" in data
        assert "ended" in data["error"].lower()
        assert mock_env_instance.step_async.call_count == 1  # still 1

    def test_step_exception_returns_error(self) -> None:
        """If env.step_async() raises, the endpoint should catch and return error."""
